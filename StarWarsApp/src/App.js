  ...
  const fetchMoviesHandler = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        "https://starwarsmovieapp-default-rtdb.asia-southeast1.firebasedatabase.app/movies.json"
      );
      ...
      console.log(data);
      {
        /* firebase에서 전송받은 객체에 암호화된 ID부여해 객체생성 + key또한 암호화된 값으로 설정하고 실제 데이터는 이 key에 대한 값으로 객체형태로 이루어짐 */
      }
